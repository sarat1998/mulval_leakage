...
def compute_trusts(fairness, goodness, principals):
    trusts = {}
    nodes = random.sample(fairness.keys(), len(principals))
...
def leakage_in_prolog(fairness, goodness, input_path):
    try:
        f = open(input_path, 'a+')
        principals = set(re.findall(r'(\w*victim\w*)', f.read()))
        trusts = compute_trusts(fairness, goodness, principals)
...
if __name__ == '__main__':
    if len(sys.argv) == 2:
        G = create_graph("network.csv")
        fairness, goodness = compute_fairness_goodness(G)
        leakage_in_prolog(fairness, goodness, sys.argv[1])
    else:
